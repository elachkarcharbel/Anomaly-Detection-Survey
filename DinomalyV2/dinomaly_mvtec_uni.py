...
def train(item_list):
    ...
    torch.save(model.state_dict(), os.path.join(args.save_dir, args.save_name, 'model_final.pth'))
...
if __name__ == '__main__':
    ...
    # Get the folder names inside the specified directory
    if os.path.exists(args.data_path):
        item_list = [folder for folder in os.listdir(args.data_path) if
                     os.path.isdir(os.path.join(args.data_path, folder))]
    else:
        print(f"Error: The path '{args.data_path}' does not exist.")
        item_list = []

    print("Folders found:", item_list)
