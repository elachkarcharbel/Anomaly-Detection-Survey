...
def run_experiment(args):
    ...
    for sub_class in categories:
        ...
        # Only create train_loader when we actually need it (avoid empty dataset errors for eval-only)
        ...
        # For domain-shift evaluation (MVTec → VisA) override the class and use MVTec 'cable' model
        if args.dataset == "visa":
            load_class = "cable"   # <-- use this MVTec class' checkpoint for all VisA evals
            print(f"Loading pretrained MVTec model from class '{load_class}' for VisA evaluation.")
        else:
            load_class = sub_class

        unet_model, seg_model = load_trained_models(config, load_class, device, which='best')
