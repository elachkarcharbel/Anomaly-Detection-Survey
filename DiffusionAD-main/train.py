...
def train(training_dataset_loader, testing_dataset_loader, args, data_len,sub_class,class_type,device ):
    ...
    patience = 2  # stop after 2 eval rounds without improvement
    ...
    for epoch in tqdm_epoch:
        ...
        if (epoch + 1) % 50 == 0 and epoch > 0:
            ...
            # Check improvement
            current_score = temp_image_auroc + temp_pixel_auroc
            best_score = best_image_auroc + best_pixel_auroc
            ...
            if current_score > best_score:
                ...
                print(f" Improvement at epoch {epoch}: Image AUROC {best_image_auroc}, Pixel AUROC {best_pixel_auroc}")
            else:
                epochs_without_improvement += 1
                print(f" No improvement ({epochs_without_improvement}/{patience}) for {sub_class}")

            # Early stopping condition
            if epochs_without_improvement >= patience:
                print(f" Early stopping triggered for {sub_class} at epoch {epoch}")
                break
...
def eval(testing_dataset_loader,args,unet_model,seg_model,data_len,sub_class,device):
    ...
    try:
        ...
        print(f"[WARN] Only one class in ground truth for {sub_class}, skipping AUROC.")
        auroc_image = float('nan')

    try:
        auroc_pixel = round(roc_auc_score(total_pixel_gt, total_pixel_pred), 3) * 100
    except ValueError:
        auroc_pixel = float('nan')
...
def save(unet_model, seg_model, args, final, epoch, sub_class):
    ...
    folder_path = f'{args["output_path"]}/model/diff-params-ARGS={args["arg_num"]}/{sub_class}'
    
    # ensure the folder exists
    os.makedirs(folder_path, exist_ok=True)

    # build the file path
    file_path = os.path.join(folder_path, f'params-{final}.pt')
...
def load_trained_models(args, sub_class, device, which='best'):
    folder_path = f'{args["output_path"]}/model/diff-params-ARGS={args["arg_num"]}/{sub_class}'
    file_path = os.path.join(folder_path, f'params-{which}.pt')
