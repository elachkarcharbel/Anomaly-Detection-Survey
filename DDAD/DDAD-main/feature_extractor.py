...
def domain_adaptation(unet, config, fine_tune):
    ...
    if fine_tune:      
        ...
        feat_name = f"feat{config.model.DA_chp}"
        feat_path = os.path.join(checkpoint_dir, feat_name)

        if os.path.exists(feat_path):
            checkpoint = torch.load(feat_path, map_location="cpu")
            feature_extractor.load_state_dict(checkpoint)
            print(f" → Loaded DA checkpoint: {feat_name}")
        else:
            # fallback: try epoch folder
            epoch_dir = os.path.join(checkpoint_dir, str(config.model.DA_chp))
            candidates = ["last.pth", "model.pth", "checkpoint.pth"]
            found = None
            for cand in candidates:
                cand_path = os.path.join(epoch_dir, cand)
                if os.path.exists(cand_path):
                    found = cand_path
                    break

            if found is not None:
                checkpoint = torch.load(found, map_location="cpu")
                feature_extractor.load_state_dict(checkpoint)
                print(f" → Loaded DA checkpoint from {found}")
            else:
                # Skip DA gracefully
                print(f"  No DA checkpoint found for {config.data.category}, skipping domain adaptation.")
                return feature_extractor  # untrained feature extractor
