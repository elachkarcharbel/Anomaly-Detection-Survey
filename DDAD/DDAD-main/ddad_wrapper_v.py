...
def run_experiment(args):
    ...
    if args.dataset.lower() == "mvtec":
        config.data.data_dir = "datasets/MVTec"
    elif args.dataset.lower() == "visa":
        config.data.data_dir = "datasets/visa"
    else:
    ...
    for category in categories:
        ...
        if latest_step is not None:
            ...
            # --- Patch dataset paths for VisA ---
            if args.dataset.lower() == "visa":
                import shutil, glob

                visaclass_dir = os.path.join(config.data.data_dir, category)
                normal_src = os.path.join(visaclass_dir, "Data/Images/Normal")
                anomaly_src = os.path.join(visaclass_dir, "Data/Images/Anomaly")
                train_good = os.path.join(visaclass_dir, "train/good")
                test_broken = os.path.join(visaclass_dir, "test/broken")

                # Create expected directories
                os.makedirs(train_good, exist_ok=True)
                os.makedirs(test_broken, exist_ok=True)

                # Helper function to copy all images with common extensions
                def copy_images(src_dir, dst_dir):
                    ...
                    for f in files:
                        ...
                        if not os.path.exists(dst):
                            shutil.copy(f, dst)
                    print(f" → Populated {dst_dir} from {src_dir} ({len(files)} images)")
                                                                                                            

                # Populate train and test
                if len(glob.glob(os.path.join(train_good, "*"))) == 0:
                    copy_images(normal_src, train_good)
                if len(glob.glob(os.path.join(test_broken, "*"))) == 0:
                    copy_images(anomaly_src, test_broken)

                # Fix checkpoint dir for VisA
                config.model.checkpoint_dir = os.path.join("checkpoints", "VisA")
